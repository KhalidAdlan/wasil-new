            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ ($order->product ? $order->product->id : old('product_id')) == $id ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('salesmen') ? 'is-invalid' : '' }}" name="salesmen_id" id="salesmen_id">
                    @foreach($salesmens as $id => $salesmen)
                        <option value="{{ $id }}" {{ ($order->salesmen ? $order->salesmen->id : old('salesmen_id')) == $id ? 'selected' : '' }}>{{ $salesmen }}</option>
                    @endforeach
                </select>

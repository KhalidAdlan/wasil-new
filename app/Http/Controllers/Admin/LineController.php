...
class LineController extends Controller
{
    ...
    public function show(Line $line)
    {
        ...
        $waitingOrders = \App\Order::all()->where('deleted_at','=', null)->where('state', '!=', 'ملغية')->where('line_id',$line->id)->where('status','انتظار')->groupBy('invoice_number');
        $shippedProducts = \App\Order::all()->where('deleted_at','=', null)->where('state', '!=', 'ملغية')->where('line_id',$line->id)->where('status','تم الشحن')->groupBy('product_id');
